...
/**
 * Set 是ES6新增的一种数据结构。它类似于数组，但是值是唯一的，没有重复值。
 * (一般可以利用这个特性，进行数值数组的去重，也可以用于字符串数组)
 * Set 本身是一个构造函数，生成set数据结构。
 *
 * Set 内部的比较两个值是否不同，使用的算法叫做“Same-value-zero equality”，类似于 ===，
 * 和 === 相区别的就是 NaN, Set 加入值时认为 NaN 等于自身
 *
 */
