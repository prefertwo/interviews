 ...
 *      call 和 apply 的区别在于，参数。第一个参数都是要改变上下文的对象。call 从第二个参数开始，以参数列表的形式展现。apply 则把后面的参数全部放在一个数组里作为第二个参数。
 */
...
// js 中的伪数组 具有length属性，可以通过 下标 来访问元素，但没有 array 的 push、pop 等方法。利用 call、apply 转化为真正的数组就可以使用数组的方法了。
